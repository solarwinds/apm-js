...
if (collector) {
  ...
  env.SW_APM_TRUSTED_PATH = "/swotel/docker/collector/server-grpc.crt"
}
...
const dockerEnv = Object.entries(env)
  ...
  .join(" ")
exec(
  `docker compose -f docker/docker-compose.yml run ${dockerEnv} -e PORT=8080 -p 8080:8080 --rm 20-alpine 'cd ./examples/${example} && yarn install && (yarn start || true)'; yarn install`,
)
