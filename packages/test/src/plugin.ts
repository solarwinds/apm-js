...
function withoutUndefinedProperties(source: object, deep: boolean, copy = true): object {
	const object = copy
		? (Object.create(
	...
	for (const key of Reflect.ownKeys(object)) {
		...
		if (descriptor.enumerable && descriptor.configurable && descriptor.value === undefined) {
...
function loosely<B extends Chai.Equal | Chai.Include>(base: B): B {
	return new Proxy(base, {
		apply(
			...
			return base.call(assertion, withoutUndefinedProperties(value, deep), message)
		},
	})
}
