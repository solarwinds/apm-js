...
JsContext::JsContext(swo::CallbackInfo const info)
    : swo::Class<JsContext, Context, swo::ClassType::Static>(info) {}
...
Napi::Value JsContext::setTracingMode(swo::CallbackInfo const info) {
    auto newMode = info.arg<int>(0);
    Context::setTracingMode(newMode);
    return info.undefined();
}

Napi::Value JsContext::setTriggerMode(swo::CallbackInfo const info) {
    auto newMode = info.arg<int>(0);
    Context::setTriggerMode(newMode);
    return info.undefined();
}

Napi::Value JsContext::setDefaultSampleRate(swo::CallbackInfo const info) {
    auto newRate = info.arg<int>(0);
    Context::setDefaultSampleRate(newRate);
    return info.undefined();
}

Napi::Value JsContext::getDecisions(swo::CallbackInfo const info) {
    ...
    Context::getDecisions(
        ...
        options.get_optional<swo::NullableString>("in_xtrace").value_or(std::nullopt).data(),
        options.get_optional<swo::NullableString>("tracestate").value_or(std::nullopt).data(),
        ...
        options.get_optional<swo::NullableString>("header_options").value_or(std::nullopt).data(),
        options.get_optional<swo::NullableString>("header_signature").value_or(std::nullopt).data(),
...
Napi::Value JsContext::get(swo::CallbackInfo const info) {
    return JsMetadata::js_new(info, new Metadata(Context::get()));
}

Napi::Value JsContext::toString(swo::CallbackInfo const info) {
    return info.value(Context::toString());
}

Napi::Value JsContext::set(swo::CallbackInfo const info) {
    auto md = JsMetadata::Unwrap(info.arg<Napi::Object>(0));
    Context::set(md->base->metadata());
    return info.undefined();
}

Napi::Value JsContext::fromString(swo::CallbackInfo const info) {
    auto s = info.arg<std::string>(0);
    Context::fromString(s);
    return info.undefined();
}

Napi::Value JsContext::copy(swo::CallbackInfo const info) {
    return JsMetadata::js_new(info, Context::copy());
}

Napi::Value JsContext::setSampledFlag(swo::CallbackInfo const info) {
    Context::setSampledFlag();
    return info.undefined();
}

Napi::Value JsContext::clear(swo::CallbackInfo const info) {
    Context::clear();
    return info.undefined();
}

Napi::Value JsContext::isValid(swo::CallbackInfo const info) {
    return info.value(Context::isValid());
}

Napi::Value JsContext::isSampled(swo::CallbackInfo const info) {
    return info.value(Context::isSampled());
}

Napi::Value JsContext::validateTransformServiceName(swo::CallbackInfo const info) {
    auto service_key = info.arg<std::string>(0);
    return info.value(Context::validateTransformServiceName(service_key));
}

Napi::Value JsContext::shutdown(swo::CallbackInfo const info) {
    Context::shutdown();
    return info.undefined();
}

Napi::Value JsContext::isReady(swo::CallbackInfo const info) {
    auto timeout = info.arg<unsigned int>(0);
    return info.value(Context::isReady(timeout));
}

Napi::Value JsContext::isLambda(swo::CallbackInfo const info) {
    return info.value(Context::isLambda());
}

Napi::Value JsContext::createEvent(swo::CallbackInfo const info) {
...
Napi::Value JsContext::startTrace(swo::CallbackInfo const info) {
    return JsEvent::js_new(info, Context::startTrace());
}

Napi::Value JsContext::createEntry(swo::CallbackInfo const info) {
    auto md = JsMetadata::Unwrap(info.arg<Napi::Object>(0));
...
Napi::Value JsContext::createExit(swo::CallbackInfo const info) {
