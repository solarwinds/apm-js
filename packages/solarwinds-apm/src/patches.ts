...
function instrumentationPatcher<const Name extends keyof InstrumentationConfigMap>(
	names: readonly Name[],
	patch: (config: NonNullable<InstrumentationConfigMap[Name]>, options: Options) => void,
...
function envPatcher(
	...
	patch: (value: string | undefined, config: Configuration) => string | undefined,
) {
...
const INSTRUMENTATION_PATCHERS = [
	...
	instrumentationPatcher(["@opentelemetry/instrumentation-aws-lambda"], (config) => {
		config.enabled ??= environment.IS_AWS_LAMBDA
	}),

	instrumentationPatcher(["@opentelemetry/instrumentation-aws-sdk"], (config) => {
		if (environment.IS_AWS_LAMBDA) {
			config.enabled ??= true
		}
	}),

	instrumentationPatcher(["@opentelemetry/instrumentation-cassandra-driver"], (config) => {
	...
	instrumentationPatcher(["@opentelemetry/instrumentation-http"], (config, options) => {
	...
	instrumentationPatcher(["@opentelemetry/instrumentation-mysql2"], (config, options) => {
		config.addSqlCommenterCommentToQueries ??= options.insertTraceContextIntoQueries
	}),

	instrumentationPatcher(["@opentelemetry/instrumentation-pg"], (config, options) => {
		config.requireParentSpan ??= true
		config.addSqlCommenterCommentToQueries ??= options.insertTraceContextIntoQueries
	}),

	instrumentationPatcher(["@opentelemetry/instrumentation-runtime-node"], (config, options) => {
		config.enabled ??= options.runtimeMetrics
	}),

	instrumentationPatcher(
		["@opentelemetry/instrumentation-dns", "@opentelemetry/instrumentation-net"],
		(config) => {
			config.enabled ??= false
		},
	),
