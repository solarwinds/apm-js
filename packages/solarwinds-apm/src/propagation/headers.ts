...
import { contextStorage } from "../storage.ts"
import { swValue } from "./trace-context.ts"
 ...
 *
 * This propagator SHOULD NOT be registered with the tracer provider since it is meant for
 * response and not request. Currently it is registered in the HTTP instrumentation response
 * hook until an official response propagation API is added.
 */
...
/** Returns the first element if the value is an array or the value as-is otherwise. */
...
/**
 * Returns the result of {@link Array.join} if the value is an array or the value as-is
 * otherwise.
 *
