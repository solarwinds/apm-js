...
export const schema = (defaults: Defaults) =>
	v.pipe(
		v.object({
			...
			collector: v.optional(schemas.url, "apm.collector.na-01.cloud.solarwinds.com"),

			logLevel: v.optional(schemas.logLevel, "warn"),
			tracingMode: v.optional(schemas.tracingMode),
			triggerTraceEnabled: v.optional(schemas.boolean, defaults.triggerTraceEnabled),
		...
		v.transform((raw): Configuration => {
			const service = getStringFromEnv("OTEL_SERVICE_NAME") ?? raw.serviceKey.name
...
export const env = {
	...
	/**
	 * Returns a config object from an environment object by filtering the keys by prefix then
	 * stripping it and renaming them to camelCase.
	 */
...
/** Computes the collector endpoint for the given signal based on the config and environment. */
function signalEndpoint(signal: Signal, collector: URL): string {
	const path = `/v1/${signal}`

	const specific = getStringFromEnv(`OTEL_EXPORTER_OTLP_${signal.toUpperCase()}_ENDPOINT`)
	if (specific) {
		return specific
	}
	...
	endpoint.hostname = collector.hostname.replace(/^apm\.collector\./, "otel.collector.")
