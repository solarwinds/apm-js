...
import { type Context, createContextKey, diag, type Span } from "@opentelemetry/api"
...
export interface ContextStorage<T> {
	 ...
	 *
	 * Note that while the context object itself is immutable, reference types stored in it are
	 * not. This means it is possible to modify the return value of this function in-place if it
	 * is a reference type.
	 */
	 ...
	 *
	 * Note that the context object is immutable and this function returns an updated context
	 * without modifying the given one in-place. in-place.
	 */
	 ...
	 *
	 * Note that the context object is immutable and this function returns an updated context
	 * without modifying the given one in-place.
	 */
 ...
 *
 * This makes it possible to attach arbitrary data to OTel spans without altering the span
 * object.
 *
 * The stored value will be accessible throughout the entire lifecycle of the span from the
 * moment it is set until the span finishes being exported.
 *
 * Unlike the previous implementation which used the span ID as a key and needed to be carefully
 * cleared at the right times to avoid leaking memory, this uses a WeakMap with the span object
 * itself as a key which ensures the stored values are kept at least as long as they need to be
 * accessed.
 *
 * However it it still useful to manually clear unused entries to avoid using up memory
 * unnecessarily while waiting for the next GC cycle.
 */
...
/** Promise which resolves to the value passed to it when calling its resolve method. */
	...
	return {
		...
		set: (span, val) =>
			withSdkSpan(span, false, (span) => {
				const storage = GLOBAL_SPAN_STORAGE.get(span) ?? new Map<symbol, unknown>()
				storage.set(key, val)
