...
import { HEADERS_STORAGE } from "../propagation/headers.ts"
import {
	ATTR_HTTP_METHOD,
	ATTR_HTTP_SCHEME,
	...
	ATTR_HTTP_TARGET,
	ATTR_NET_HOST_NAME,
} from "../semattrs.old.ts"
import { type Configuration } from "../shared/config.ts"

/**
 * Returns whether a span represents an HTTP request, and if so some useful attributes of the
 * request.
 *
...
export function httpSpanMetadata(kind: SpanKind, attributes: Attributes) {
	...
	const method = String(attributes[ATTR_HTTP_REQUEST_METHOD] ?? attributes[ATTR_HTTP_METHOD])
	const status = Number(
		attributes[ATTR_HTTP_RESPONSE_STATUS_CODE] ?? attributes[ATTR_HTTP_STATUS_CODE] ?? 0,
	)

	const scheme = String(attributes[ATTR_URL_SCHEME] ?? attributes[ATTR_HTTP_SCHEME] ?? "http")
	const hostname = String(
		attributes[ATTR_SERVER_ADDRESS] ?? attributes[ATTR_NET_HOST_NAME] ?? "localhost",
	)
 ...
 *
 * This extends {@link OboeSampler} to retrieve local settings from the configuration and manage
 * request headers retrieval and response headers setting. The only piece of logic left to
 * implement by specific sampler is remote setting retrieval.
 */
export abstract class Sampler extends OboeSampler {
	...
	constructor(config: Configuration, logger: DiagLogger, initial?: Settings) {
		...
		if (config.tracingMode !== undefined) {
			this.#tracingMode = config.tracingMode ? TracingMode.ALWAYS : TracingMode.NEVER
		}
	...
	protected override localSettings(
		...
		const identifier = meta.http ? meta.url : `${SpanKind[spanKind]}:${spanName}`
	...
	protected override setResponseHeaders(headers: ResponseHeaders, context: Context): void {
