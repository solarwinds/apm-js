...
import { type BatchObservableResult, metrics, ValueType } from "@opentelemetry/api"
import {
	ATTR_CPU_MODE,
...
export function enableRuntimeMetrics() {
	...
	const contextSwitches = meter.createObservableCounter(METRIC_PROCESS_CONTEXT_SWITCHES, {
		description: "Number of times the process has been context switched.",
		unit: "{context_switch}",
		valueType: ValueType.INT,
	})
	const pagingFaults = meter.createObservableCounter(METRIC_PROCESS_PAGING_FAULTS, {
	...
	function callback(result: BatchObservableResult) {
		...
		if (process.platform !== "win32") {
			result.observe(contextSwitches, usage.voluntaryContextSwitches, {
				[ATTR_PROCESS_CONTEXT_SWITCH_TYPE]: PROCESS_CONTEXT_SWITCH_TYPE_VALUE_VOLUNTARY,
			})
			result.observe(contextSwitches, usage.involuntaryContextSwitches, {
				[ATTR_PROCESS_CONTEXT_SWITCH_TYPE]: PROCESS_CONTEXT_SWITCH_TYPE_VALUE_INVOLUNTARY,
			})

			result.observe(pagingFaults, usage.minorPageFault, {
				[ATTR_SYSTEM_PAGING_FAULT_TYPE]: SYSTEM_PAGING_FAULT_TYPE_VALUE_MINOR,
			})
			result.observe(pagingFaults, usage.majorPageFault, {
				[ATTR_SYSTEM_PAGING_FAULT_TYPE]: SYSTEM_PAGING_FAULT_TYPE_VALUE_MAJOR,
			})
		}
