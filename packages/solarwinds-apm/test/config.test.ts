...
import { type Configuration, printError, read } from "../src/config.ts"
...
describe("read", () => {
	...
	it("properly uses OTLP env endpoints", () => {
		...
		process.env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT = "http://custom.traces.endpoint/v1/traces"
	...
	it("throws on bad boolean", () => {
		...
		expect(read)
			.to.throw(Error)
			.which.satisfies((error: unknown) => printError(error).includes("enabled"))
	})
	...
	it("throws on bad tracing mode", () => {
		...
		expect(read)
			.to.throw(Error)
			.which.satisfies((error: unknown) => printError(error).includes("tracingMode"))
	})
	...
	it("throws on non-existent trusted path", () => {
		...
		expect(read)
			.to.throw(Error)
			.which.satisfies((error: unknown) => printError(error).includes("trustedpath"))
	})
	...
	describe("transactionName", () => {
		it("supports string literals", () => {
			process.env.SW_APM_CONFIG_FILE = "test/configs/transaction-name.literal.cjs"
			const config = read()
		...
		it("supports functions", () => {
			process.env.SW_APM_CONFIG_FILE = "test/configs/transaction-name.function.cjs"
			const config = read()

			expect(config.transactionName?.({ name: "one" } as ReadableSpan)).to.equal("one")
			expect(config.transactionName?.({ name: "two" } as ReadableSpan)).to.equal("two")
		})

		it("supports schemes", () => {
			process.env.SW_APM_CONFIG_FILE = "test/configs/transaction-name.schemes.cjs"
			const config = read()
