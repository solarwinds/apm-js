...
import { OboeSampler, type SampleParams, spanType } from "../src/sampler.ts"
import {
	BucketType,
	Flags,
	type LocalSettings,
...
import { type RequestHeaders, type ResponseHeaders } from "../src/trace-options.ts"
...
interface MakeSpan {
	...
	/**
	 * Whether to generate a sw tracestate field. "inverse" will generate a field that contradicts
	 * the W3C trace flags.
	 */
	sw?: boolean | "inverse"
}
const makeSpan = (options: MakeSpan = {}): Span => {
	...
	const swFlags =
		options.sw === "inverse" ? (object.sampled ? "00" : "01") : object.sampled ? "01" : "00"
	...
	return {
		spanContext: () => ({
			...
			traceState: options.sw ? createTraceState(`sw=${object.id}-${swFlags}`) : undefined,
		}),
	} as Span
}
...
const makeSampleParams = (options: MakeSampleParams = {}): SampleParams => {
	...
	return [
		...
		object.parent ? object.parent.spanContext().traceId : randomBytes(16).toString("hex"),
		object.name,
		object.kind,
		{},
		[],
...
const makeRequestHeaders = (options: MakeRequestHeaders = {}): RequestHeaders => {
	...
	const kvs = options.kvs ? Object.entries(options.kvs).map(([k, v]) => `${k}=${v}`) : []
	...
	if (options.signature) {
		...
		headers["X-Trace-Options-Signature"] = createHmac("sha1", options.signatureKey)
...
describe("OboeSampler", () => {
	...
	describe("dice roll", () => {
		...
		it("records but doesn't sample when dice failure", async () => {
			...
			await checkCounters(["trace.service.request_count", "trace.service.samplecount"])
		})
	})
