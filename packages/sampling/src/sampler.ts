...
import { BucketType, Flags, type LocalSettings, merge, type Settings } from "./settings.ts"
...
export type SpanType = "root" | "entry" | "local"
...
/**
 * This class implements all of the core sampling logic. It is meant to be extended by specific
 * samplers that regularly call {@link updateSettings} and provide an implementation of
 * {@link localSettings}.
 *
 * For instance, a classic sampler would retrieve the settings using gRPC, while a serverless
 * sampler would retrieve them from the file on disk. By extending this class neither needs to
 * reimplement shared sampling logic.
 */
export abstract class OboeSampler implements Sampler {
	...
	shouldSample(...params: SampleParams): SamplingResult {
		...
		this.logger.debug(`span type is ${type}`)
		...
		if (s.headers["X-Trace-Options"]) {
			...
			if (s.headers["X-Trace-Options-Signature"]) {
				...
				if (s.traceOptions.response.auth !== Auth.OK) {
					this.logger.debug("X-Trace-Options-Signature invalid; tracing disabled")
	...
	/**
	 * Updates the current remote sampling settings. This should be called by the subclass
	 * whenever the remote settings are updated.
	 */
	 ...
	 * @param params - Parameters passed to {@link Sampler.shouldSample}
	 */
	protected abstract setResponseHeaders(headers: ResponseHeaders, ...params: SampleParams): void
