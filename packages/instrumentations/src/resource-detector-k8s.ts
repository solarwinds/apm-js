...
const ATTRIBUTES = [ATTR_K8S_NAMESPACE_NAME, ATTR_K8S_POD_UID, ATTR_K8S_POD_NAME]
...
export class K8sDetector implements ResourceDetector {
	...
	detect() {
		const promise = context.with(suppressTracing(context.active()), async () => {
	...
	async #podUid(): Promise<string | undefined> {
		...
		try {
			...
			for await (const line of rl) {
				...
				if (!Number.isSafeInteger(Number(id)) || !Number.isSafeInteger(Number(parentId))) {
					// invalid entry
					continue
				}
